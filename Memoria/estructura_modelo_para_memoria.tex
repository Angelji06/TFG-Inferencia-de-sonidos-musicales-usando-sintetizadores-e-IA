...
Frobenius, capturando la estrutura global del espectro (como los armónicos y la energía). Estas caractarísticas 
...
\subsection{Prototipo 5: Modelo Final y Expansión Paramétrica (Envolventes)}
...
Por esta razón, se propone un nuevo modelo en el que se explora
un espacio de parámetros mayor, ampliando la dimensionalidad de 3 a 8 parámetros. A las variables
...
Son precisamente estas caractarísticas del \textbf{Prototipo 5}, por las que es necesario una nueva 
...
\subsubsection{Generación del Dataset Aleatorio}
Para ello, el sistema genera un número (N) de muestras de audios con valores continuos aleatorios,
en base a unos rangos de valores específicos para cada parámetro. 

En primer lugar, se sintetizan los N archivos \textit{.wav} mediante la librería \textbf{NumPy} y la formulación matemática de la FM. 
Se define una frecuencia de muestreo \textit{sample rate} de 44100 Hz
y un tiempo de duración de audio de 2 segundos. Se 
incrementa el tiempo de duración (de 1 a 2 segundos) con el fin de asegurar un margen temporal
...
Posteriormente, los audios generados \textit{.wav} se convierten a tensores nativos de \textit{PyTorch}. Se cargan mediante \textit{torchaudio} y
se les aplica un suavizado de amplitud en los extremos (\textit{fade-in} y \textit{fade-out}) con el objetivo de 
evitar chasquidos o artefactos indeseados. Finalmente, se generan las dos transformaciones principales:
de audio a espectrograma y de amplitud lineal a logarítmica (dB). Esta función una
normalización de pico, calcula la STFT, extrae la magnitud y realiza su conversión a decibelios. 

\subsubsection{Adaptación de la Arquitectura (CNNRegressor5)}
