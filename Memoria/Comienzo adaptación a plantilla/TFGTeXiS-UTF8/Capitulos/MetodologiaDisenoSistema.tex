...
Con el objetivo de lograr un acercamiento a las aplicaciónes reales del \textbf{Sound Matching} con sonidos ricos y con sonoridad, se decide que el sistema debe trabajar con los siguientes parámetros:
...
Se sintetizan los N archivos \textit{.wav} mediante la librería \textbf{NumPy} y la formulación matemática de la FM. 
...
rejilla definida que el modelo acababa memorizando, incluso aplicando \textit{jitter}\footnote{Técnica de inyección de ruido de forma aleatoria en los datos.} para solventarlo.

La generación de envolventes se lleva a cabo de manera puramente matemática con \textbf{NumPy} sobre un vector de tiempo unificado ($t$). Se utiliza el mismo algoritmo geométrico para ambas envolventes, indicando un \textit{sustain} de 0.0 en la de modulación porque carece de él. 

Para el ataque, en el caso de la amplitud, se sube progresivamente el volumen de 0.0 a 1.0, y en el caso de la modulación se sube de 0.0 a 1.0 el valor que multiplica al índice de modulación. Ambos en el tiempo que va desde el segundo 0, al marcado por la variable \textit{attack}, con la fórmula $t / \textit{attack}$. 
...
\item \textbf{Extracción y guardado: }Posteriormente, se extrae la magnitud de la señal, se convierte a decibelios (dB) y se guarda como tensores (formato \textit{.pt}). Además,
