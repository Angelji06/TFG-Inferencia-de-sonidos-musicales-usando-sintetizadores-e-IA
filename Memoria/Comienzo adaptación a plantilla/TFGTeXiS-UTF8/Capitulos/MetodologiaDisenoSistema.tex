...
\textcolor{red}{FALTA mencionar los 8 parámetros (figura) y explicarlos brevemente }
...
aumento. Sin embargo esta idea nos daba problemas de overfitting \textcolor{red}{FOOTNOTE EXPLICANDO} al generar una
rejilla definida que el modelo acababa memorizando incluso aplicando jitter\textcolor{red}{FOOTNOTE EXPLICANDO} para solventarlo.
...
\textcolor{red}{Hacer esto una lista}
1. Se les aplica un suavizado de amplitud en los extremos (\textit{fade-in} y \textit{fade-out}) con el objetivo de 
evitar chasquidos o ruidos indeseados provocados por las limitaciones de la FFT. 
2. Después, una normalización de pico (peak normalization), para asegurar consistencia de intensidad de volumenes entre muestras.
3. Luego se aplican las dos transformaciones principales:
Se transforma el audio a espectrograma usando STFT y después se pasa de amplitud lineal a logarítmica (dB), en ambos casos usando Librosa. 
...
4. Posteriormente, se extrae la magnitud de la señal, se convierte a decibelios (dB) y se guarda como tensores (formato \textit{.pt}). Además,
