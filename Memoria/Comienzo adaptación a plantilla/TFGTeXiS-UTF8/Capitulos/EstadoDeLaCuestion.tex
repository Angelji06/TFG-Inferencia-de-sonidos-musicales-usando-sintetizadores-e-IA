...
En el corazón de toda síntesis sonora se encuentra la onda sinusoidal, descrita por la ecuación fundamental:
...
donde $A$ representa la \textbf{amplitud} (la altura del sonido, relacionada con el volumen), $f$ es la \textbf{frecuencia} en herzios (que determina el tono, con mayores valores produciendo sonidos más agudos), $t$ es el \textbf{tiempo}, y $\phi$ es la \textbf{fase inicial} en radianes (que define el punto de inicio de la oscilación). Esta onda senoidal pura es el elemento básico de toda síntesis, del cual se generan todos los sonidos.
...
Históricamente, han existido diversas aproximaciones a la síntesis. Las más tradicionales incluyen la síntesis aditiva, que construye sonidos complejos sumando formas de onda sinusoidales de diferentes frecuencias y amplitudes; y la síntesis sustractiva, que esculpe el sonido partiendo de una forma de onda rica en frecuencias a la que se le aplican filtros para eliminar los componentes no deseados. Estas técnicas fundamentales abrieron el camino hacia métodos más eficientes pero matemáticamente más complejos, como la FM.

\subsection{Síntesis por Modulación de Frecuencia (FM) y el Yamaha DX7}
La \textbf{síntesis FM} es una tipo de síntesis descubierta a finales de los años sesenta (Chowning, 1973). Su funcionamiento se basa en utilizar un oscilador (denominado \textbf{modulador}) para alterar la frecuencia de otro oscilador (denominado \textbf{portador}). El oído humano interpreta esta interacción como la creación de timbres completamente nuevos y armónicamente ricos.
...
\begin{equation}
x(t) = A_c \cos\left(2\pi f_c t + I \sin(2\pi f_m t)\right)
